...
This cookbook is specific for setting up chef group and users for now.

    include_recipe "users::chefs"
...
Note this LWRP searches the "users" data bag for the "chef" group attribute, and adds those users to a Unix security group "chef".  The only required attribute is group_id, which represents the numeric Unix gid and *must* be unique.  The default action for the LWRP is ":create" only.
...
Paste the user's public SSH key into the ssh_keys value. Also make sure the uid is unique, and if you're not using bash, that the shell is installed. The default search, and Unix group is chef.

The recipe, by default, will also create the chef group. If you're using the opscode sudo cookbook, they'll have sudo access in the default site-cookbooks template. They won't have passwords though, so the sudo cookbook's template needs to be adjusted so the chef group has NOPASSWD.

The chef group will be created with GID 2300. This may become an attribute at a later date.
