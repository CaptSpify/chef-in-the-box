...
# Certs
cookbook_file "/etc/dovecot/example.pem" do
  source "example.pem"
  owner "root"
  group node['dovecot']['conf_files_group']
  mode node['dovecot']['conf_files_mode']
...
%w[ /etc/dovecot /etc/dovecot/private ].each do |path|
  directory path do
    action :create
  end
end

cookbook_file "/etc/dovecot/private/example.key" do
  source "example.key"
  owner "root"
  group node['dovecot']['conf_files_group']
  mode 0600
...
cookbook_file "/etc/default/imap.passwd" do
  source "imap.passwd"
  owner "root"
  group node['dovecot']['conf_files_group']
  mode 0600
